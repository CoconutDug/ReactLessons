...
const personReducer = (state, action) => {
  if (action.type === "SET_PERSONS") return action.payload;
...
const App = () => {
  ...
  const [userList, dispatchPersons] = React.useReducer(personReducer, []);
  ...
  */
  ...
  const handleRemovePerson = (item) => {
    dispatchPersons({ type: "REMOVE_PERSON", payload: item });
  };
      ...
      */}
      ...
      <button
        onClick={(event) => {
          ...
          dispatchPersons([...userList, newUser]);
        }}
      >
        Add
      </button>
